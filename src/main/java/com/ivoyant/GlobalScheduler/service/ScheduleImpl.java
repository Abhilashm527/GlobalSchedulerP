...
public class ScheduleImpl {
    ...
    private static final String insertSQl = "INSERT INTO public.schedule(\n" +
            "\tscheduleid, name, description, target, active, targettype, cron_expression, zoneid, createdtime, lastrun, nextrun, state, projectid)\n" +
            "\tVALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    ...
    private static final String updateSQl = "UPDATE public.schedule\n" +
            "\tSET scheduleid=?, name=?, description=?, target=?, active=?, targettype=?, cron_expression=?, zoneid=?, createdtime=?, lastrun=?, nextrun=?, state=?, projectid=?\n" +
            "\tWHERE scheduleid=?";
    ...
    private static final String deleteHistoryByScheduleId = "delete from schedule_history where scheduleid= ?";
    ...
    public Schedule create(Schedule schedule) throws SQLException {
        ...
        insertStmt.setString(13,schedule.getProjectid());
        insertStmt.execute();
        return schedule;
    }
    ...
    public Schedule getSchedule(int id) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            schedule.setProjectid(resultSet.getString("projectid"));
        }
        if (schedule == null)
            return null;
        return schedule;
    ...
    public Schedule updateSchedule(Schedule schedule) throws SQLException {
        ...
        if (resultSet.next()) {
            ...
            updateStmt.setString(13,schedule.getProjectid());
            updateStmt.setInt(14, schedule.getScheduleId()); // Set the ID for the WHERE clause of the update query
    ...
    public boolean deleteSchedule(int id) {
        try {
            ...
            PreparedStatement deleteScheduleHistoryBySchedeuleId = jdbcTemplateConnection.prepareStatement(deleteHistoryByScheduleId);
            deleteScheduleById.setInt(1, id);
            deleteScheduleHistoryBySchedeuleId.setInt(1,id);
            deleteScheduleHistoryBySchedeuleId.executeUpdate();
    ...
    public HashMap<String, Object> getScheduleHistory(int id) throws SQLException {
        ...
        while (result.next()) {
            ...
            scheduleHistory.put("Project ID", result.getString("projectid"));
        }
